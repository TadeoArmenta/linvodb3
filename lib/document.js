...
function checkKey (k, v) {
  ...
  if ((k[0] === '$') && !(k === '$$date' && typeof v === 'number') && !(k === '$$regex' && typeof v === 'string') && !(k === '$$deleted' && v === true) && !(k === '$$indexCreated') && !(k === '$$indexRemoved') && !(k === '$$bignumber' && typeof v === 'string')) {
...
function serialize (obj) {
  ...
  res = JSON.stringify(obj, function (k, v) {
    ...
    if (v.isBigNumber) { return { $$bignumber: v.toString()} }
...
function deserialize (rawData) {
  return JSON.parse(rawData, function (k, v) {
    ...
    if (k === '$$bignumber') {return new BigNumber(v)}
...
function deepCopy (obj, strictKeys) {
  ...
  if (typeof obj === 'object') {
    if (obj && obj.isBigNumber) { return new BigNumber(obj) }
    res = {};
    Object.keys(obj).forEach(function (k) {
...
lastStepModifierFunctions.$inc = function (obj, field, value) {
  if (typeof value !== 'number' && !value.isBigNumber) { throw value + " must be a number"; }

  if (obj[field]  && obj[field].isBigNumber){
    const toValue = value
    if (!value.isBigNumber) {
      toValue = new BigNumber(value)
    }
    if(toValue.s == -1) {
      if (obj[field].lt(toValue.abs())) {
        throw new Error('cant minus')
      }
      obj[field] = obj[field].minus(toValue.abs())        
    } else {
      obj[field] = obj[field].plus(toValue)        
    }
...
comparisonFunctions.$elemMatch = function (obj, value) {
  if (!util.isArray(obj)) { throw "$elemMatch operator used without an array"; }
