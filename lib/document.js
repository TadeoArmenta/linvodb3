 ...
 */
function deserialize (rawData, projection = null) {
  var doc = JSON.parse(rawData, function (k, v) {
  ...
  _.each(doc, function(v, k) {
    ...
    // Apply projection $elemMatch operator
    if (Array.isArray(v) && projection._elemMatchOperators[k]) {
      var match = _.find(v, projection._elemMatchOperators[k]);
 ...
 */
function Document(db, raw, projection = null)
{
  var self = this, raw;
  if (raw===undefined) raw = { };
  if (typeof(raw)==="string") raw = deserialize(raw, projection);
