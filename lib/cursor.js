 ...
 */
Cursor.prototype.select = function(projection) {
  if (typeof projection === 'string') {
    // String form
    if (!this._projection)
      this._projection = {};

    for (var field of projection.split(' '))
      this._projection[field] = true;
  } else {
    // Object form
    this._projection = projection;

    // Initialize projection $elemMatch operator functions
    var projectionElemMatchOperators = {};
    _.each(this._projection, function(v, k) {
      if (typeof v === 'object' && '$elemMatch' in v)
        projectionElemMatchOperators[k] = function(elem) {
          return _.every(v.$elemMatch, function(emV, emK) {
            return emK in elem && elem[emK] === emV;
          });
        };
    });

    projection._elemMatchOperators = projectionElemMatchOperators;
  }
...
/**
 * Lean the request to return plain JSON objects
 * instead of Document objects
 */
Cursor.prototype.lean = function() {
  this._lean = true;
  return this;
};
...
Cursor.prototype.exec = function(callback) {
  ...
  var stream = Cursor.getMatchesStream(this.db, this.query, sort, this._prefetched, this._projection, this._lean);
 ...
 */
Cursor.getMatchesStream = function(db, query, sort, prefetched, projection = null, lean = false) {
  var sort = sort || {};
  ...
  stream.on("ids", stream.trigger = function(ids) {
    ...
    ids.forEach(function(id, i) {
      db._retrQueue.push(function(cb) {
        Cursor.retriever({ stream: stream, id: id, idx: i, db: db, prefetched: prefetched && prefetched[id], projection: projection, lean: lean }, function() {
...
Cursor.retriever = function(task, cb) {
  ...
  task.db.store.get(task.id, function(err, buf) {
    ...
    if (err) task.stream.emit("error", err);
      ...
      val: function() {
        return task.lean ? document.deserialize(buf, task.projection) : new task.db(buf, task.projection);
      },
      lock: function() {
...
Cursor.getIdsForQuery = function(db, query, sort) {
  ...
  var indexable = true; // Query cannot be indexed
