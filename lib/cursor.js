...
Cursor.retriever = function(task, cb) {
  ...
  // task.db._pipe and task.db._retrQueue should be empty
  // ugly workaround; TODO: fix
  if (task.db.store.isClosed())
    return cb("Database is not open, Cursor.retriever");
  ...
  function match(key, val) {
    ...
    if (key[0] == "$") {
      if (key=="$and" || key=="$or") {
        ...
        else if (_.findIndex(i, function(o) { return !o || !o._indexed; })>-1) indexable = false;
