...
function isNormalized(spec) {
    return !isComplexSpec(spec) && !util.isArray(spec) && ('schema' in spec && 'type' in spec);
}
...
function mapSpec(spec)
{
	...
	if (spec === ObjectId) return "ObjectId";
	return spec;
};
...
function normalize(schema) {
	_.each(schema, function(spec, key) {
		...
		if (isComplexSpec(spec)) {
            if (util.isArray(spec.type)) {
                schema[key] = _.assign({}, {
                    schema: mapSpec(spec.type[0]) || undefined,
                    type: "array"
                }, _.pickBy(spec, function(specValue, specKey) {
                    return specKey !== "type" && specAllowedKeys.indexOf(specKey) !== -1;
                }));
            } else {
...
function getIndexes(obj, prefix) {
  ...
  {
  		if (spec.schema && typeof spec.schema === "object") return indexes = indexes.concat(getIndexes(spec.schema, prefix+key+"."));
