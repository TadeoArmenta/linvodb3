...
function mapSpec(spec)
{
	...
	if (spec.name == 'BigNumber') return "bignumber";
	return spec;
};
	...
	_.each(schema, function(spec, key) {
		...
		if (isComplexSpec(spec)) {
            if (util.isArray(spec.type)) {
                schema[key] = { schema: mapSpec(spec.type[0]) || undefined, type: "array" };
            } else {
...
function canCast(val, spec)
{
  ...
  if (spec == 'bignumber' && val.toString) return true;
  return false;
};
